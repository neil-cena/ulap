...
/**
 * Bug Reproduction Test — BackupViewModel.syncNow().
 *
 * Contract: syncNow() must do all three of the following (subject to the Wi-Fi guard):
 *   1. Call fetchIndex() to pull the latest backup index from Telegram so that files
 *      backed up on other devices appear in the local timeline immediately.
 *   2. Call scanMedia(fullScan=false) to pick up any new local files.
 *   3. Call BackupForegroundService.startBackup(context) to upload pending items.
 *
 * Regression tracked: commit 0ed0b9c accidentally dropped fetchIndex() when adding the
 * upload pipeline and Wi-Fi guard — restoring it is the defect this BRT covers.
 *
...
class BackupViewModelSyncNowBrt {
    ...
    // ─── Upload service: BackupForegroundService.startBackup(context) must be called ──
    ...
    // ─── Cross-device sync: fetchIndex() must be called ─────────────────────

    /**
     * syncNow() must call fetchIndex() so that files backed up on other devices are
     * merged into the local DB and become visible in the timeline immediately.
     *
     * Regression: commit 0ed0b9c dropped this call when adding the upload pipeline.
     *
     * FAILS against current code: fetchIndex() is never invoked.
     */
    @Test
    fun syncNow_callsFetchIndex_forCrossDeviceSync() = runTest {
        whenever(userPrefs.wifiOnly).thenReturn(MutableStateFlow(false))
        setupWifiConnectivity()
        whenever(fetchIndex()).thenReturn(Result.success(0))
        val viewModel = buildViewModel()

        Mockito.mockConstruction(Intent::class.java).use {
            viewModel.syncNow()

            // FAILS against current code: fetchIndex() is never called
            verify(fetchIndex).invoke()
        }
    }

    // ─── Wi-Fi guard: blocked on mobile data when wifiOnly = true ───────────
    ...
    fun syncNow_wifiOnly_mobileData_doesNotCallScanMedia() = runTest {
        ...
        // calls scanMedia() unconditionally        verify(scanMedia, never()).invoke(any())
    }
